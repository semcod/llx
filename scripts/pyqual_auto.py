...
def main() -> int:
    ...
    parser.add_argument(
        "--model", "-m", required=True, help="LLM model to use (e.g., openrouter/openai/gpt-5-mini)"
    )
    parser.add_argument(
        "--force-fix", "-f", action="store_true", help="Force fix stage by lowering quality gates"
    )
    parser.add_argument(
        "--dry-run", "-n", action="store_true", help="Dry run (don't actually run pyqual)"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument(
        "--workdir", "-w", type=Path, default=Path("."), help="Working directory (default: .)"
    )
    parser.add_argument(
        "--no-restore", action="store_true", help="Don't restore original config after run"
    )
