...
class BehavioralPasswordDetector:
    ...
    def _load_learning(self) -> None:
        ...
        try:
            ...
            self.user_naming_conventions = defaultdict(int, data.get("naming_conventions", {}))
        except Exception:
    ...
    def _analyze_file(self, content: str, stats: dict) -> None:
        ...
        password_vars = re.finditer(
            r"\b(\w*(?:password|passwd|pwd|secret|key|token|auth)\w*)\b", content, re.IGNORECASE
        )
        ...
        assignments = re.finditer(r'(\w+)\s*=\s*["\']([^"\']+)["\']', content)
    ...
    def _update_learned_patterns(self, stats: dict) -> None:
        ...
        sorted_conventions = sorted(
            stats["naming_conventions"].items(), key=lambda x: x[1], reverse=True
        )
    ...
    def report_false_positive(
        self, text: str, context: str, reason: str, timestamp: str = None
    ) -> None:
        ...
        self.false_positives.append(
            FalsePositiveRecord(text=text, context=context, reason=reason, timestamp=timestamp)
        )
    ...
    def calculate_adaptive_confidence(
        self, text: str, var_name: str, base_confidence: float
    ) -> float:
    ...
    def generate_custom_patterns(self) -> list[str]:
        ...
        top_conventions = sorted(
            self.user_naming_conventions.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def get_learning_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_false_positives": [
                {"text": fp.text, "reason": fp.reason} for fp in self.false_positives[-5:]
            ],
        }
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for name, count in sorted(
            learn_stats["naming_conventions"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for var_name, value, base_conf in test_cases:
            adaptive = detector.calculate_adaptive_confidence(value, var_name, base_conf)
