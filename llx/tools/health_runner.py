...
class HealthCheckRunner:
    ...
    def _generate_recommendations(self, health_report: Dict[str, Any]) -> List[str]:
        ...
        stopped_containers = [
            name for name, health in containers.items() if health.get("status") != "running"
        ]

        if stopped_containers:
            recommendations.append("Start containers: `./docker-manage.sh dev`")
    ...
    def _check_services(self, health_report: Dict[str, Any]) -> None:
        ...
        for service_name in self.checker.endpoints.keys():
            ...
            response_time = (
                f" ({service_health.get('response_time', 0):.0f}ms)"
                if service_health.get("response_time")
                else ""
            )
    ...
    def _check_system_resources(self, health_report: Dict[str, Any]) -> None:
        ...
        print(
            f"  🧠 Memory: {system_resources['memory']['available'] // 1024}GB available / {system_resources['memory']['total'] // 1024}GB total"
        )
        print(
            f"  💾 Disk: {system_resources['disk']['available']}GB available / {system_resources['disk']['total']}GB total"
        )
    ...
    def _check_network(self, health_report: Dict[str, Any]) -> None:
        ...
        print(
            f"  {internet_icon} Internet: {'Connected' if network_health['internet'] else 'Disconnected'}"
        )
