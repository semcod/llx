...
class ModelSelector:
    ...
    def _determine_tier(self, model_id: str, tier_name: str) -> str:
        ...
        if tier_name not in self._LITELLM_MODEL_KEYS:
            return tier_name
        if (
            "nemotron" in model_id
            or "llama-3.2-3b-instruct:free" in model_id
            or "mistral-7b-instruct:free" in model_id
        ):
            return "free"
...
OPENROUTER_FREE_FILTER = ModelFilter(provider=ModelProvider.OPENROUTER, tier=ModelTier.FREE)
