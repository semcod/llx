...
class LLXStrategyBuilder:
    ...
    def __init__(self, llx_path: str = "llx", model: str = "qwen2.5:3b", local: bool = True):
        """
    ...
    def ask_llm_questions(self) -> Dict[str, Any]:
        ...
        answers["quality_goals"] = self._call_llm(
            "Ask user: What are the quality goals for this project? List 3-5 bullet points."
        )
        ...
        answers["project_type"] = self._call_llx(
            "Ask user: What type of project is this? Choose one: web, mobile, api, desktop, library"
        )
        ...
        answers["sprints"] = []
        sprint_count = int(self._call_llx("Ask user: How many sprints do you want to plan? (1-5)"))

        for i in range(1, sprint_count + 1):
            objectives = self._call_llm(
                f"Ask user: What are the objectives for sprint {i}? List up to 5 bullet points."
            )

            sprint_name = self._call_llm(f"Ask user: What is the name for sprint {i}?")
        ...
        answers["feature_pattern"] = {"title": feature_title, "description": feature_desc}
        ...
        answers["bug_pattern"] = {"title": bug_title, "description": bug_desc}
        ...
        answers["tech_debt_pattern"] = {"title": tech_debt_title, "description": tech_debt_desc}
        ...
        has_gates = (
            self._call_llx("Ask user: Do you want to define quality gates? (yes/no)").lower()
            == "yes"
        )

        if has_gates:
            gate_count = int(self._call_llx("Ask user: How many quality gates? (1-3)"))

            for i in range(gate_count):
                gate_name = self._call_llx(f"Ask user: What is the name of quality gate {i + 1}?")
                gate_desc = self._call_llx(f"Ask user: Describe quality gate '{gate_name}'.")
    ...
    def _parse_bullet_list(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if line.startswith(("- ", "*", "•", "1.", "2.", "3.", "4.", "5.")):
                ...
                clean = (
                    line[2:] if line.startswith(("- ", "*", "•")) else line.split(".", 1)[1].strip()
                )
    ...
    def answers_to_strategy(self, answers: Dict[str, Any]) -> Strategy:
        ...
        patterns.append(
            TaskPattern(
                ...
                priority="highest",
                model_hints=ModelHints(triage="balanced", implementation="local"),
            )
        )
        ...
        patterns.append(
            TaskPattern(
                ...
                priority="medium",
                model_hints=ModelHints(implementation="local"),
            )
        )
        ...
        strategy_data = {
            ...
            "metadata": {"generated_by": "llx-strategy-builder", "model": self.model},
        }

        # Validate and return
        return Strategy.model_validate(strategy_data)

    def build_strategy(self, output_path: Optional[str] = None) -> Strategy:
        """
...
def create_strategy_command(
    output: str = "strategy.yaml", model: str = "qwen2.5:3b", local: bool = True
):
    """
