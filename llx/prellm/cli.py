...
def query(
    ...
    small: Optional[str] = typer.Option(
        None, "--small", "-s", help="Small LLM for preprocessing (default: from .env)"
    ),
    large: Optional[str] = typer.Option(
        None, "--large", "-l", help="Large LLM for execution (default: from .env)"
    ),
    strategy: Optional[str] = typer.Option(
        None,
        "--strategy",
        "-S",
        help="Strategy: classify|structure|split|enrich|passthrough (default: from .env)",
    ),
    context: Optional[str] = typer.Option(
        None, "--context", "-C", help="User context tag (e.g. 'gdansk_embedded_python')"
    ),
    config: Optional[Path] = typer.Option(None, "--config", "-c", help="Optional YAML config file"),
    memory: Optional[Path] = typer.Option(
        None, "--memory", "-m", help="Path to UserMemory database"
    ),
    codebase: Optional[Path] = typer.Option(
        None, "--codebase", help="Path to codebase root for context indexing"
    ),
    collect_env: bool = typer.Option(
        False, "--collect-env", help="Collect full shell environment context"
    ),
    compress_folder: Optional[Path] = typer.Option(
        None, "--compress-folder", help="Compress folder for context (path)"
    ),
    no_sanitize: bool = typer.Option(
        False, "--no-sanitize", help="Disable sensitive data filtering (dev only)"
    ),
    show_schema: bool = typer.Option(
        False, "--show-schema", help="Show generated context schema (debug)"
    ),
    show_blocked: bool = typer.Option(
        False, "--show-blocked", help="Show blocked sensitive fields"
    ),
    json_output: bool = typer.Option(False, "--json", "-j", help="Output as JSON"),
    trace: bool = typer.Option(
        False, "--trace", "-t", help="Generate markdown execution trace (.prellm/)"
    ),
    trace_dir: Optional[Path] = typer.Option(
        None, "--trace-dir", help="Trace output directory (default: .prellm)"
    ),
...
def process(
    ...
    _process_cmd(
        config=config, guard_config=guard_config, dry_run=dry_run, json_output=json_output, env=env
    )
...
def session_list_cmd(
    memory: Path = typer.Option(
        ".prellm/user_memory.db", "--memory", "-m", help="Path to memory database"
    ),
):
...
def session_export_cmd(
    ...
    session_id: Optional[str] = typer.Option(
        None, "--id", help="Session ID (default: auto-generated)"
    ),
):
