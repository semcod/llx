...
- `plan_code(strategy, output_dir, profile)` — Generate project code from strategy using LLM (free tier by default).
- `plan_run(project_dir, port, install)` — Start the generated FastAPI application with uvicorn.
