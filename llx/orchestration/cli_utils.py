...
def cmd_remove_pair_wrapper(
    ...
    if not first_value or not second_value:
        print(
            f"❌ --{first_attr.replace('_', '-')} and --{second_attr.replace('_', '-')} required for remove"
        )
        return False
    ...
    if success:
        print(
            f"✅ {first_label} '{first_value}' / {second_label} '{second_value}' removed successfully"
        )
        if save_func:
            save_func()
    else:
        print(
            f"❌ Failed to remove {first_label.lower()} '{first_value}' / {second_label.lower()} '{second_value}'"
        )
...
def cmd_status_wrapper(
    args: Any, id_attr: str, id_label: str, status_func: Callable[[str], dict]
) -> bool:
...
def cmd_list_wrapper(
    items: list, title: str, formatter: Optional[Callable[[Any], str]] = None
) -> bool:
...
def cmd_cleanup_wrapper(cleanup_func: Callable[[], int], item_label: str = "items") -> bool:
