                ...
                if health_endpoint:
                    try:
                        response = requests.get(
                            f"http://localhost:{port}{health_endpoint}", timeout=5
                        )
        ...
        try:
            ...
            for line in lines[1:]:
                ...
                if len(parts) >= 3:
                    ...
                    usage_info[service_name] = {"cpu": cpu_percent, "memory": memory_usage}
    ...
    def print_status_summary(self, env: str = "dev"):
        ...
        if usage:
            ...
            for service, info in usage.items():
                print(
                    f"  📊 {service}: CPU {info.get('cpu', 'N/A')}, Memory {info.get('memory', 'N/A')}"
                )
