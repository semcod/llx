...
from .models import Strategy, TaskType
...
class SimpleStrategyBuilder:
    ...
    def __init__(self, model: Optional[str] = None, config_file: Optional[Path] = None):
        """Initialize builder.
    ...
    def build_from_analysis(
        self, project_path: str, focus: str = "complexity", sprints: int = 3
    ) -> Strategy:
    ...
    def _generate_strategy_content(self, metrics: Any, focus: str, sprints: int) -> Dict[str, Any]:
        ...
        for i in range(sprints):
            ...
            sprint = {
                ...
                "tasks": [
                    f"task-{j + 1}" for j in range(len(self._get_tasks_for_focus(focus, sprint_id)))
                ],
            }
            ...
            "patterns": [
                {
                    ...
                    "priority": p.priority,
                    "model_hints": {
                        "design": p.model_hints.design.value
                        if hasattr(p.model_hints.design, "value")
                        else p.model_hints.design,
                        "implementation": p.model_hints.implementation.value
                        if hasattr(p.model_hints.implementation, "value")
                        else p.model_hints.implementation,
                        "review": p.model_hints.review.value
                        if hasattr(p.model_hints.review, "value")
                        else p.model_hints.review,
                    },
                }
