...
def _generate_tasks_from_patterns(task_patterns):
    ...
    for task_name in task_patterns:
        task_str = (
            task_name.get("name", str(task_name)) if isinstance(task_name, dict) else str(task_name)
        )
        task_type = (
            "tech_debt"
            if "refactor" in task_str.lower() or "extract" in task_str.lower()
            else "feature"
        )
...
def _normalize_goal(data):
    ...
    if not goal:
        goal = {
            "title": data.get("project_name", "Project Refactoring"),
...
def generate_strategy_with_fix(
    project_path,
...
def _print_generation_info(project_path, model, sprints, focus, description=None):
    ...
    console.print("  Using OpenRouter free models")


def _build_strategy_prompt(
    metrics, sprints, focus, description=None, project_type=None, framework=None
):
    ...
    try:
        ...
        template = config["strategy"]["prompts"].get(
            template_key, config["strategy"]["prompts"]["api"]
        )
        ...
        console.print(
            f"[yellow]Warning: Could not load config, using default template ({e})[/yellow]"
        )
        return f"""
...
def _call_llm_for_strategy(prompt, model):
    ...
    response = client.chat(messages=[ChatMessage(role="user", content=prompt)], model=model)

    console.print("[green]Got response from LLM[/green]")

    # Debug: save full response
    with open("/tmp/llm_response.txt", "w") as f:
        f.write(response.content)
    console.print("[dim]Full response saved to /tmp/llm_response.txt[/dim]")
...
def _parse_and_fix_yaml(content):
    ...
    try:
        with open("/tmp/extracted_yaml.txt", "w") as f:
            f.write(yaml_text)
        console.print("[dim]Extracted YAML saved to /tmp/extracted_yaml.txt[/dim]")
    except Exception:
        pass
...
def _fix_yaml_formatting(yaml_text):
    ...
    try:
        with open("/tmp/fixed_yaml.txt", "w") as f:
            f.write(yaml_text)
        console.print("[dim]Fixed YAML saved to /tmp/fixed_yaml.txt[/dim]")
    except Exception:
        pass
        ...
        if line.strip().startswith("- ") and i < len(lines) - 1:
            ...
            if next_line and not next_line.startswith("-") and not next_line.startswith(" "):
                ...
                if any(
                    ...
                    for word in ["add", "run", "enforce", "introduce", "apply"]
                ):
                    fixed_lines.append("")
...
def _fix_indentation(yaml_text):
    ...
    for line in lines:
        ...
        if line.startswith("name:") and not line.startswith("  name:"):
            ...
            if (
                corrected_lines
                and corrected_lines[-1].strip()
                and not corrected_lines[-1].startswith(" ")
            ):
...
def main():
    ...
    try:
        ...
        console.print("3. If satisfied, apply without --dry-run")
