    ...
    for model in models_to_try:
        try:
            ...
            strategy_data = generate_strategy_with_fix(
                project_path=project_path, model=model, sprints=3, focus="improvement"
            )
            ...
            console.print(
                f"  Tasks: {sum(len(s.get('tasks', [])) for s in strategy_data.get('sprints', []))}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", default="strategy.yaml", help="Output file (default: strategy.yaml)"
    )
