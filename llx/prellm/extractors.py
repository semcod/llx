...
def extract_matched_rule_from_state(
    state: dict, current_missing_fields: list[str]
...
def build_decomposition_result(
    ...
    strategy = (
        DecompositionStrategy(pipeline_name)
        if pipeline_name in strategy_values
        else DecompositionStrategy.CLASSIFY
    )
...
def format_classification_context(prep_result: Any) -> list[str]:
    ...
    if isinstance(classification, dict):
        ...
        domain = classification.get("domain", "general")
        parts.append(f"User intent: {intent} (confidence: {confidence}, domain: {domain})")
