...
class EntropyAnalyzer:
    ...
    def analyze_randomness(self, text: str, context: str = "") -> EntropyResult:
        ...
        is_random = entropy >= self.HIGH_ENTROPY_THRESHOLD or (
            entropy >= self.MEDIUM_ENTROPY_THRESHOLD and likely_type != "noise"
        )

        return EntropyResult(
            text=text, entropy=entropy, is_random=is_random, likely_type=likely_type
        )
...
class MLBasedAnonymizer:
    ...
    def _detect_pii_semantic(self, text: str) -> list[dict]:
        ...
        name_patterns = [
            ...
            (
                r'(?:name|full\s*name)\s*[=:]\s*["\']?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)',
                "name_assignment",
            ),
        ...
        address_patterns = [
            (
                r"\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln)",
                "street_address",
            ),
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for finding in ml_findings[:5]:
            print(
                f"  - {finding['type']}: {finding['text'][:30]}... (entropy: {finding.get('entropy', 'N/A')})"
            )
        ...
        for rep in replacements[:5]:
            print(
                f"  {rep['mask']} ← {rep['original'][:30]}... (confidence: {rep['confidence']:.0%})"
            )
