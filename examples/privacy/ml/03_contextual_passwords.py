...
class ContextualPasswordDetector:
    ...
    def _compile_patterns(self) -> list[tuple]:
        ...
        pattern1 = re.compile(rf'\b({indicators})\b\s*=\s*["\']?([^"\'\s#;]+)', re.IGNORECASE)
        compiled.append(("assignment", pattern1))

        # Pattern 2: Dictionary key
        pattern2 = re.compile(
            rf'["\']?({indicators})["\']?\s*:\s*["\']?([^"\'}}\]\s]+)', re.IGNORECASE
        )
        compiled.append(("dict_key", pattern2))

        # Pattern 3: Function parameter
        pattern3 = re.compile(rf'\b({indicators})\s*=\s*["\']([^"\']+)["\']', re.IGNORECASE)
        compiled.append(("parameter", pattern3))

        # Pattern 4: Constructor/init
        pattern4 = re.compile(rf'self\.({indicators})\s*=\s*["\']?([^"\'\s]+)', re.IGNORECASE)
    ...
    def _calculate_confidence(
        self, password: str, var_name: str, context_type: str, context: str
    ) -> float:
